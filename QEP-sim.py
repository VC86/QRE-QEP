...
if __name__ == "__main__":
    ...
    for tt in range(ntri):
        ...
        for ss in range(msteps):
            ...
            red_train,_ = quant_rand_embed(train_data.T,m=M[ss],delta=delta0,rngseed=rseed)
            red_test,_ = quant_rand_embed(test_data.T,m=M[ss],delta=delta0,rngseed=rseed)
            ...
            red_train,_ = quant_rand_embed(train_data.T,m=M[ss],delta=delta1,rngseed=rseed)
            red_test,_ = quant_rand_embed(test_data.T,m=M[ss],delta=delta1,rngseed=rseed)
            red_train = red_train.T
            red_test = red_test.T
            KNN.fit(red_train,ctrain)
            ...
            red_train,_ = quant_rand_embed(train_data.T,m=M[ss],delta=delta2,rngseed=rseed)
            red_test,_ = quant_rand_embed(test_data.T,m=M[ss],delta=delta2,rngseed=rseed)
            red_train = red_train.T
            red_test = red_test.T
            KNN.fit(red_train,ctrain)
    ...
    plt.plot(np.log2(M*1.0/N),mqrp_perr_1,label="$QRP_{m,\delta=0.25 \sigma}$", marker='o')
    plt.plot(np.log2(M*1.0/N),mqrp_perr_2,label="$QRP_{m,\delta=\sigma}$", marker='o')
